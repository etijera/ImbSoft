...
namespace Referencias
{
    ...
    public class ConexionDB
    {
        ...
        private void Conectar()
        {
            ...
            if (length >= 1)
            {
                conexion = new SqlConnection(appconfig.ConnectionStrings.ConnectionStrings[length].ConnectionString);
                conexion.Open();
            }
            else
            {
        ...
        {
            appconfig = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
            connStringSettings = appconfig.ConnectionStrings.ConnectionStrings[cstrName];
            if (connStringSettings == null)
                return null;
            else
