...
namespace UsuarioControles
{
    public partial class Fecha : UserControl
    {
        ...
        public bool Validar()
        {
            ...
            if (ValidarPeriodo)
            {
                if (MesPeriodo != TxtFecha.DateTime.Month || AñoPeriodo != TxtFecha.DateTime.Year)
                {
                    ...
                    XtraMessageBox.Show("El documento que intenta crear no corresponde al periodo que se esta trabajando (" + mes + " de " + AñoPeriodo + "). Por favor verifique.", Referencias.Properties.Resources.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    TxtFecha.Focus();
                    return false;
                }
            }
            ...
            if (ValidarCierre)
            {
                ...
                {
                    String mes = Funciones.getInstancia().Numero2Mes(MesPeriodo);
                    XtraMessageBox.Show("Error, el período (" + mes + " de " + AñoPeriodo + ") está cerrado. Por favor verifique.", Referencias.Properties.Resources.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    TxtFecha.Focus();
                    return false;
                }
            }
        ...
        private void TxtFecha_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                XtraMessageBox.Show("Debe ingresar una fecha", Referencias.Properties.Resources.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                TxtFecha.Focus();
                return;
            }
            else
